...
def start(paras_father):
    ...
    f=open(ids_txt)
    if ids_txt=="E:/qkj/split/north-west/ids.txt":
        datas=map(lambda x:int(x),f.readlines()[0][1:-1].split(','))
        start_index=datas.index(47647)
        paras_son=map(lambda x:[in_dem,in_envelope,filedir+"/"+str(x)+"/"],datas[start_index+1:])
        f.close()
    else:
...
if __name__=='__main__':
    ...
    paras=map(lambda x:[dem,envelope,x],ids_path[2:])
    #print(paras)
    map(start,paras)
    ################################ end ##################################
